...
class ProductInListRepository:
    ...
    async def get_products_in_list(self, list_id: int):
        query = "SELECT pil.product_in_list_id, pil.quantity_type, pil.quantity, pil.price, pil.product_id FROM shoplist_db.ProductInList pil JOIN Product p ON pil.product_id = p.product_id WHERE pil.list_id = :list_id;"
    ...
    async def create_product_in_list(
        ...
        query = "INSERT INTO shoplist_db.ProductInList (list_id, product_id, quantity_type, quantity, price) VALUES (:list_id, :product_id, :quantity_type, :quantity, :price);"
    ...
    async def update_product_in_list(
        ...
        query = "UPDATE shoplist_db.ProductInList SET product_id = :product_id, quantity_type = :quantity_type, quantity = :quantity, price = :price WHERE list_id = :list_id AND product_in_list_id = :product_in_list_id;"
        return await database.execute(
            query,
            {
                "list_id": list_id,
